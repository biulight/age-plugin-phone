...
use crate::protocol::{
    Error, Id, MAX_REQUEST_LIFETIME_SECS, PairingRecord, ProtocolDigest, ProtocolNonce,
};
...
mod file {
    ...
    fn acquire_lock(path: &Path) -> Result<File, Error> {
        age_plugin_phone_platform_storage::unix::acquire_lock(&sibling_path(path, ".lock")?)
            .map_err(|_| Error::ReplayState)
    }
    fn read_private_file(path: &Path) -> Result<Vec<u8>, Error> {
        age_plugin_phone_platform_storage::unix::read_private_file(path, MAX_STATE_BYTES)
            .map_err(|_| Error::ReplayState)
    }
    fn persist_create(path: &Path, encoded: &[u8]) -> Result<(), Error> {
        age_plugin_phone_platform_storage::unix::persist_create(path, encoded)
            .map_err(|_| Error::ReplayState)
    }
    fn atomic_replace(path: &Path, encoded: &[u8]) -> Result<(), Error> {
        age_plugin_phone_platform_storage::unix::atomic_replace(path, encoded)
            .map_err(|_| Error::ReplayState)
    }
...
mod windows_file {
    ...
    use age_plugin_phone_platform_storage::windows::{
...
mod tests {
    ...
    mod unix {
        ...
        use super::*;
        use crate::protocol::FileReplayGuard;
    ...
    mod windows {
        ...
        use age_plugin_phone_platform_storage::windows::ensure_private_directory;
