...
2026-09-10 范围调整：用户当前没有 GUI 调用需求，明确暂不验收 GUI 应用及其网络、摄像头和密钥访问权限。该范围延期、不记通过，也不阻塞当前终端 CLI 验收；不建立 GUI 支持声明。后续恢复 GUI 需求时再完成相应矩阵。该 GUI 决定本身不调整安全要求；后续用户另行批准的跨平台 replay 边界见上文。

2026-09-10 补充范围决定：因 QR 非当前主流程，用户明确本版本跳过刚提出的摄像头权限拒绝/撤销验收。该项延期、不记通过，不影响已完成的内建摄像头 QR 功能证据；不扩大为豁免所有 QR 或其他安全项目。外接 UVC 可用性尚未确认，仍单独记录。
...
- 明确备份排除、迁移助理和 Time Machine 恢复策略。复制到另一台设备必须因硬件绑定失败。
  Windows/macOS 均不保证识别同机恢复的旧有效 replay 快照；增强保护按用户决定转入后续跨平台 POC。
  备份排除或硬件密钥不是防回滚证明；原反例继续保留。旧响应仍须被新会话拒绝，每次手机私钥操作仍须新原生验证。
