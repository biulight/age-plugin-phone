...
Implementation snapshot `f1e1be1` includes M1/M2 storage and keys, M3 managed/explicit
...
| Desktop source installation | Four detached crates tested and registry-installed at `f1e1be1` with Rust 1.88; later documentation/test-only snapshot still needs final archive refresh |
...
| Windows regression | Mac-to-MSVC cross-check stopped in mozjpeg-sys native C compilation (`-fPIC` unsupported); neither full Windows compilation nor runtime is established |
...
| Exact final archive and installed digest | Refresh after final source/documentation changes |
| Terminal → age/rage → installed plugin | Real phone flow pending; public synthetic interoperability can run independently |
