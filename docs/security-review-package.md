...
Source locations below follow ADR 0024. Historical review results remain evidence
for their original candidate and do not attest to the current refactor.

| Review area | Normative design | Primary Rust implementation and tests | Primary Android implementation and tests |
| --- | --- | --- | --- |
| P-256 age recipient and file-key wrap | [ADR 0001](adr/0001-experimental-p256-recipient.md), [ADR 0012](adr/0012-private-stanza-selection.md) | `crates/core/src/recipient/mod.rs`, `crates/core/src/recipient/plugin.rs`, `crates/core/test-vectors/p256-recipient-v1.json`, `crates/core/test-vectors/p256-recipient-v2.json` | `TaggedRecipientCrypto.kt`, `TaggedRecipientCryptoTest.kt` |
| Canonical pairing transcript and fingerprints | [ADR 0002](adr/0002-experimental-offline-envelope.md), [ADR 0008](adr/0008-bidirectional-pairing.md), [ADR 0014](adr/0014-split-desktop-key-protocol-v2.md) | `crates/core/src/protocol/mod.rs`, `crates/desktop/src/pairing.rs`, `crates/core/test-vectors/pairing-transcript-v2.json` | `PairingConfirmationSession.kt`, `NativePairingResponseController.kt` and their tests |
| Long-term P-256 role separation | [ADR 0002](adr/0002-experimental-offline-envelope.md), [ADR 0013](adr/0013-windows-cng-key-boundaries.md), [ADR 0014](adr/0014-split-desktop-key-protocol-v2.md) | Transcript checks in `crates/core/src/protocol/mod.rs`, strict stub checks in `crates/desktop/src/pairing.rs`, and custody in `crates/platform-keys/src/windows.rs` | Transcript checks in `OfflineEnvelopeCrypto.kt`, state checks in `PairingStateStore.kt`, and custody in `PhoneIdentityKeyStore.kt` |
| Private stanza selection | [ADR 0012](adr/0012-private-stanza-selection.md) | `crates/core/src/recipient/mod.rs`, `crates/desktop/src/age_identity.rs` | `TaggedRecipientCrypto.kt` structural and unwrap validation |
| Signed, encrypted response envelope | [ADR 0002](adr/0002-experimental-offline-envelope.md), [ADR 0009](adr/0009-one-shot-qr-unwrap.md) | `crates/core/src/protocol/mod.rs`, `crates/desktop/src/unwrap.rs`, `crates/core/test-vectors/offline-envelope-v2.json` | `OfflineEnvelopeCrypto.kt`, `NativeUnwrapResponseController.kt` and tests |
| Windows and Android replay storage | [ADR 0003](adr/0003-persistent-replay-state.md), [ADR 0004](adr/0004-android-pairing-state.md), [ADR 0015](adr/0015-windows-private-storage.md) | `crates/core/src/protocol/replay.rs`, `crates/platform-storage/src/windows.rs`, `crates/desktop/src/locator.rs` | `PairingStateStore.kt`, `PairingStateStoreTest.kt` |
| CNG/TPM custody and Windows private files | [ADR 0013](adr/0013-windows-cng-key-boundaries.md), [ADR 0015](adr/0015-windows-private-storage.md) | `crates/platform-keys`, `crates/platform-storage`, Windows paths in `crates/desktop` | Not applicable |
| Android StrongBox custody and per-use authorization | [ADR 0007](adr/0007-android-production-key-custody.md), [ADR 0009](adr/0009-one-shot-qr-unwrap.md), [StrongBox PoC](android-strongbox-poc.md) | Portable verifier and operation traits in `crates/core/src/protocol` and `crates/core/src/recipient` | `PhoneIdentityKeyStore.kt`, `PhoneIdentityPlugin.kt`, `NativeUnwrapResponseController.kt` and tests |
| QR and common stream framing | [ADR 0005](adr/0005-qr-framing.md), [ADR 0006](adr/0006-native-qr-capture.md), [ADR 0011](adr/0011-desktop-native-qr-scanner.md), [ADR 0016](adr/0016-common-transport-and-adb-alpha.md) | `crates/core/src/protocol/qr.rs`, `crates/desktop/src/transport.rs`, `crates/desktop/src/qr_scanner.rs` | `QrFraming.kt`, `QrScanSession.kt`, `NativeQrScannerController.kt`, `StreamTransport.kt` and tests |
